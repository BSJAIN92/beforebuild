...
export const dashboard = query({ args: {}, handler: async ctx => {
  ...
  return { day, users: usage.length, totals, rejected: rejected.map(({ _id, email, ideaId, tier, text, reason, source, createdAt }) => ({ id: _id, email, ideaId, tier, text, reason, source, createdAt })) };
} });
