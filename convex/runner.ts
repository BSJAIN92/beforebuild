...
interface WorkState { idea: Idea; stage: string; responseId?: string; polls: number; researchKind?: string; inputMessageId?: string; }
export const work = internalAction({ args: { id: v.id("ideas"), token: v.string() }, handler: async (ctx, args): Promise<void> => {
  ...
  try {
    ...
    const turn = await ai.interview(idea);
    ...
    if (error instanceof AIResponseValidationError && workState) await ctx.runMutation(internal.jobs.recordAIResponseError, { ...args, provider: process.env.AI_PROVIDER || "openai", model: (process.env.AI_PROVIDER === "gemini" ? process.env.GEMINI_MODEL : process.env.AI_INTERVIEW_MODEL) || "default", operation: "interview", tier: workState.idea.tier, answerCount: workState.idea.answerCount, finish: false, message: error.message, prompt: JSON.stringify({ instructions: interviewInstructions(workState.idea), input: interviewInput(workState.idea) }), ...error.diagnostic });
