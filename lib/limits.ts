...
export function resolveUsageLimit(tier: Tier, kind: UsageKind, environment: Record<string, string | undefined>): number {
  const config = USAGE_LIMITS[tier][kind]; if (!config) return 0;
  const value = Number(environment[config.env] || config.fallback);
