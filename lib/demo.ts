...
export function createDemoBackend(): Backend {
  let data: Snapshot = { ideas: [], viewer: { email: "founder@demo.local", name: "Alex", admin: true, demo: true }, pricing: { enabled: false, currency: "USD", intermediate: 0, advanced: 0 }, invites: [] };
  ...
  try {
    ...
    if (raw) {
      ...
      if (Array.isArray(parsed.ideas) && parsed.ideas.every(i => i && typeof i.id === "string" && typeof i.description === "string" && i.canvas && Array.isArray(i.messages))) {
        data = { ...data, ideas: parsed.ideas, pricing: parsed.pricing || data.pricing, invites: parsed.invites || [] };
