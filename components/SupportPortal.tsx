...
function ConnectedSupport() {
  ...
  return <main className="support-shell"><section className="support-card"><Brand /><div className="support-heading"><div><span className="eyebrow">VERIFIED EMAIL SUPPORT</span><h1>How can we help?</h1><p>Your reply will go to <strong>{viewer.email}</strong>. This does not add you to the beta waitlist.</p></div><Link className="text-button" href={access.allowed ? "/" : "/sign-in"}>{access.allowed ? "Back to workspace" : "Back"}</Link></div>
    ...
    {viewer.admin ? <section className="support-admin"><h2>Submitted requests</h2><p>Only admins can see and update these messages.</p><div className="support-list">{submissions?.map(item => <article key={item._id}><div><strong>{item.queryType}</strong><span>{item.name} · {item.email}</span><time>{new Date(item.createdAt).toLocaleString()}</time></div><p>{item.message}</p><label>Status<select value={item.status} onChange={event => void setStatus({ id: item._id, status: event.target.value as SupportStatus })}><option>Not started</option><option>In progress</option><option>Completed</option></select></label></article>)}{submissions === undefined ? <p>Loading requests…</p> : submissions.length === 0 ? <p>No support requests yet.</p> : null}</div></section> : null}
  </section></main>;
}
