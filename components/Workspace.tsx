...
import type { AbuseDashboard, Backend, Invite, Pricing, SupportSubmission, UsageLimits, Viewer, WaitlistEntry } from "../lib/backend";
...
export function LiveWorkspace({ viewer }: { viewer: Viewer }) {
  ...
  const waitlist = useQuery(api.access.listWaitlist, viewer.admin ? {} : "skip") as WaitlistEntry[] | undefined;
  const support = useQuery(api.support.list, viewer.admin ? {} : "skip") as (Omit<SupportSubmission, "id"> & { _id: string })[] | undefined;
  ...
  useEffect(() => { if (ideas && pricing && usageLimits) backend.push({ ideas, pricing, usageLimits, invites: invites || [], waitlist: waitlist || [], support: (support || []).map(({ _id, ...item }) => ({ id: _id, ...item })), abuse, viewer }); }, [backend, ideas, pricing, usageLimits, invites, waitlist, support, abuse, viewer]);
